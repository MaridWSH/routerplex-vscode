...
test("adds and replaces a managed POSIX export", () => {
  const first = applyManagedEnvironment("export PATH=/usr/bin\n", "ROUTERPLEX_API_KEY", "sk-first", "posix");
  const second = applyManagedEnvironment(first, "ROUTERPLEX_API_KEY", "sk-second", "posix");

  assert.match(second, /export ROUTERPLEX_API_KEY='sk-second'/);
  assert.doesNotMatch(second, /sk-first/);
  assert.equal((second.match(/RouterPlex managed environment/g) ?? []).length, 2);
});

test("writes fish syntax when needed", () => {
  assert.equal(
    environmentExportLine("ROUTERPLEX_API_KEY", "sk-test", "fish"),
    "set -gx ROUTERPLEX_API_KEY 'sk-test'",
  );
});

test("removes only the managed environment block", () => {
  const source = applyManagedEnvironment("export EXISTING=value\n", "ROUTERPLEX_API_KEY", "sk-test", "posix");
